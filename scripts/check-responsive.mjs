...
try {
  ...
  assert.deepEqual(bogoState.half,{value:'',hidden:true,disabled:true,required:false,offerA:'Offer A Checkout total $28.00 Number of items 4 Effective price per item $7.00 Savings from promotion $4.00 1 complete promotion group applied. The remaining 1 item cost full price.'},'50% off clears custom input and applies 50% automatically');
  ...
  assert.deepEqual(bogoState.free,{value:'',hidden:true,disabled:true,required:false,offerA:bogoState.hundredPercent},'Free clears custom input and applies 100% automatically');
  assert.deepEqual(bogoState.halfAgain,bogoState.half,'Switching from Free to 50% off keeps the custom field cleared and applies only the active mode');
